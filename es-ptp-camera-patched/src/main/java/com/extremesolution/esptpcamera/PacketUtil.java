...
public class PacketUtil {
    ...
    public static String hexDumpToString(byte[] a, int offset, int len) {
        ...
        for (int i = 0; i < lines; ++i) {
            b.append(String.format(Locale.US, "%04x ", i * 16));
            for (int k = 0; k < 16; ++k) {
                b.append(String.format(Locale.US, "%02x ", a[offset + i * 16 + k]));
            }
            for (int k = 0; k < 16; ++k) {
        ...
        if (rest != 0) {
            b.append(String.format(Locale.US, "%04x ", lines * 16));
            for (int k = 0; k < rest; ++k) {
                b.append(String.format(Locale.US, "%02x ", a[offset + lines * 16 + k]));
            }
